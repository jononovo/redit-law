...
- `app/api/v1/stripe-wallet/balance/sync/route.ts` — sync endpoint with 5-min cooldown, reconciliation transactions
...
#### 2b. Add implementations (find the existing `crossmintUpdateWalletBalance` method and add after it):
  ...
  try {
    ...
    if (changed) {
      ...
      await storage.crossmintCreateTransaction({
        ...
        status: "confirmed",
        quantity: 1,           // required field, default
        orderStatus: null,     // not a purchase
        metadata: {
...
**⚠️ CRITICAL — `crossmintCreateTransaction` has required fields that differ from `privyCreateTransaction`:**

The `crossmint_transactions` table has extra columns (`quantity`, `orderStatus`, `productLocator`, `productName`, `shippingAddress`, `trackingInfo`, `crossmintOrderId`). When inserting a reconciliation transaction, set `quantity: 1` and leave the commerce-specific fields as null/undefined. Check the `InsertCrossmintTransaction` type to confirm which fields are required vs optional. If `quantity` has a `.default(1)` in schema, it can be omitted. If not, it must be included.
...
async function handleSyncBalance(wallet: WalletInfo) {
  ...
  if (lastSync && Date.now() - lastSync < 5 * 60 * 1000) {
    const remaining = Math.ceil((5 * 60 * 1000 - (Date.now() - lastSync)) / 60000);
    toast({
      title: "Please wait",
      description: `Try again in ${remaining} minute${remaining > 1 ? "s" : ""}.`,
    });
    return;
  }
  ...
  try {
    ...
    if (res.status === 429) {
      ...
      toast({
        title: "Please wait",
        description: `Try again in ${Math.ceil((data.retry_after || 300) / 60)} minutes.`,
      });
      return;
    }
...
| `server/storage.ts` | Add `crossmintUpdateWalletBalanceAndSync` + `crossmintUpdateWalletSyncedAt` to interface and implementation | 2 |
...
3. Wait 5 minutes, click again → should work
