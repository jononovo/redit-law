...
> **Note:** `card_status` mirrors wallet status and is reserved for future virtual card support.
...
| `inactive` | Owner claimed but no wallet record exists (edge case) |
  ...
  "payment_links": [
    {
      ...
      "expires_at": "2026-02-07T15:00:00Z",
      "paid_at": "2026-02-06T16:45:00Z"
    }
  ]
}
```
...
| Status | Meaning |
|--------|---------|
| `pending` | Awaiting payment (checkout_url still active) |
| `completed` | Payment received, funds credited to wallet |
| `expired` | 24-hour window passed without payment |
