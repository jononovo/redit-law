...
import { ArrowLeft, Shield, Activity, Clock, Loader2, RefreshCw } from "lucide-react";
...
export default function SelfHostedCardDetailPage() {
  ...
  const fetchBotInfo = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        const bot = (data.bots || []).find((b: BotInfo) => b.bot_id === botId);
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      const res = await authFetch(`/api/v1/rail4/obfuscation/history?bot_id=${botId}&limit=100`);
      if (res.ok) {
        const data = await res.json();
  ...
  useEffect(() => {
    Promise.all([fetchCardStatus(), fetchBotInfo(), fetchEvents()]).finally(() => {
      setLoading(false);
    });
  }, [fetchCardStatus, fetchBotInfo, fetchEvents]);
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-neutral-900" data-testid="text-card-detail-title">
            {botInfo?.bot_name || botId.slice(0, 12)}
          </h1>
          <p className="text-sm text-neutral-500 font-mono">{botId}</p>
        </div>
      </div>
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                >
                  {cardStatus?.status === "active" ? "Active" : cardStatus?.status || "Unknown"}
                </Badge>
              </div>
            </div>
          </CardContent>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-neutral-900 mt-1" data-testid="text-detail-created">
                  {cardStatus?.created_at ? new Date(cardStatus.created_at).toLocaleDateString() : "—"}
                </p>
              </div>
            </div>
          </CardContent>
