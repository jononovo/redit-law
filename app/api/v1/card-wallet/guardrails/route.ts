...
import { setCrossmintGuardrailsSchema } from "@/shared/schema";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = setCrossmintGuardrailsSchema.safeParse(body);
    if (!parsed.success) {
    ...
    console.error("POST /api/v1/card-wallet/guardrails error:", error);
    return NextResponse.json({ error: "internal_error" }, { status: 500 });
  }
}
